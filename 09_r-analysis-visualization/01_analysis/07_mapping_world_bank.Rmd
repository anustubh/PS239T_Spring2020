...
p_load(tidyverse, data.table, rmarkdown, knitr, tinytex, magrittr,
       ...
       sf, 
       rnaturalearthdata #shapefile for world map
       )
```
Notes:
...
## 2.2. Create object with tidied shapefile  
...
## 3.2. Detour: How to evalute packages

There are at least two packages for downloading world bank data directly into R: 
```{r, eval=F}
?wb
?WDI
```
...
## 3.3. Select world bank data to map
```{r}
# (Optional) Rename selected results 
wb_dat <- wb_wb
...
```{r}
# Save raw data
write_csv(wb_dat, path="../02_data/data-raw/wb_gender_vars.csv")
```

```{r, eval=F}
# Load raw data if necessary 
wb_dat <- read_csv(path="../02_data/data-raw/wb_gender_vars.csv")
```
