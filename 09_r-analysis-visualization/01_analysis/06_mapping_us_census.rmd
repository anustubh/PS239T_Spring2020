...
### Dataset part 1: lat-long coordinates and shapefile IDs

# Save 'tidy' version of shapefile 
US_counties_tidy <- data.table(broom::tidy(US_counties)) 

### Dataset part 2: geography names, stardard ID vars like FIPS, and shapefile IDs

# Save row IDs in a column 
county_data <- 
  # Combine shapefile IDs with names 
  cbind(id=rownames(county_data), #shapefile IDs
        US_counties@data) %>% #names 
